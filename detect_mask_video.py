...
def detect_and_predict_mask(frame, faceNet, maskNet):
    ...
    for i in range(0, detections.shape[2]):
        ...
        if confidence > 0.5:
            ...
            # extract the face ROI, convert BGR to RGB, resize to 224x224,
            # and preprocess it
...
while True:
    # grab the frame and resize to max width of 400 pixels
    frame = vs.read()
    frame = imutils.resize(frame, width=400)

    # detect faces and determine if they are wearing a mask
    (locs, preds) = detect_and_predict_mask(frame, faceNet, maskNet)

    # loop over the detected face locations and their predictions
